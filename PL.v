...
Axiom n9_01 : ∀ F : Set → Prop,
  (¬(∀ x : Set, F x)) = (∃ x : Set, ¬ F x).

Axiom n9_02 : ∀ F : Set → Prop,
  ¬(∃ x : Set, F x) = (∀ x : Set, ¬ F x).

Axiom n9_03 : ∀ F : Set → Prop, ∀ P : Prop,
  ((∀x:Set, F x) ∨ P) = (∀x:Set, F x ∨ P).

Axiom n9_04 : ∀ F : Set → Prop, ∀ P : Prop,
  (P ∨ (∀x:Set, F x)) = (∀x:Set, P ∨ F x).

Axiom n9_05 : ∀ (F : Set -> Prop) (P : Prop),
  ((∃ x : Set, F x) ∨ P) = (∃ x : Set, F x ∨ P).

Axiom n9_06 : ∀ F : Set → Prop, ∀ P : Prop,
  (P ∨ (∃x:Set, F x)) = (∃x:Set, P ∨ F x).

Axiom n9_1 : ∀x : Set, ∀ F : Set → Prop, (F x → (∃y : Set, F y)).

Axiom n9_11 : ∀x y : Set, ∀ F : Set→Prop, ((F x ∨ F y) → (∃z : Set, F z)).
...
Proof. intros y F.
  ...
  intro n2_1a.
  specialize n9_1 with y F.
  intro n9_1a.
  replace (F y) with (~(F y) ∨ F y) in n9_1a.
  MP n9_1a n2_1a.
  replace (∃ y : Set, F y) with (∃ x : Set, ~(F x) ∨ F y) in n9_1a.
  replace (∃ x : Set, ¬(F x) ∨ F y) with ((∃ x : Set, ¬(F x)) ∨ F y) in n9_1a
    by now apply n9_05.
  replace (∃ x : Set, ~(F x)) with (~(∀ x : Set, F x)) in n9_1a
    by now apply n9_01.
...
Proof. intros Phi y.
  ...
  MP n2_1a n9_1a.
  (** Step 3 **)
  specialize n9_05 with (fun x : Prop => ~ Phi x) (Phi y). intros n9_05a. cbn in n9_05a.
